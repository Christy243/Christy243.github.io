...
$req='SELECT * FROM membre ORDER BY idmembre DESC';
$res=$pdo->prepare($req);
$res->execute();
?>
...
<input type="Search"  placeholder="Search for DateAdhesion" name="dateAdhesion" class="form-control me-2">
...
require("cnx.php"); 
$req=("SELECT * FROM membre WHERE dateAdhesion='$dateAdhesion'");
$res=$pdo->prepare($req);
$res->execute();
echo"<b>Resultat de la recherche par dateAdhesion:$dateAdhesion<br/>";
...
echo'<thead class=\"bg-dark text-light\">';
   ...
   <td><img src="<?php echo "./images/".$mb['photo'];?>" width="100" height="100"/>
    ...
    <td><a href="editMembre.php?idmembre=<?php echo($mb['idmembre']) ?>" class="btn btn-success" >Edit</a></td>
    <td><a onclick="return confirm('Etes vous sur de vouloir supprimer?'); "href="deleteMembre.php?idmembre=<?php echo($mb['idmembre']) ?>" class="btn btn-danger" >Delete</a></td>
			<td><a href="Saisiecompte.php" class="btn btn-primary">CreerCompte</a></td>	
...
<div class="bg-white">Les adhesions</div>
...
<tbody>
	<?php while ($mb=$res->fetch()) {?>
		<tr>
			<td data-title="Id"><?php echo($mb['idmembre']) ?> </td>
			...
			<td data-title="Adresse"><?php echo($mb['adresse']) ?> </td>
			...
			<td><a href="editMembre.php?idmembre=<?php echo($mb['idmembre']) ?>" class="btn btn-success" >Edit</a></td>
			<td><a onclick="return confirm('Etes vous sur de vouloir supprimer?'); "href="deleteMembre.php?idmembre=<?php echo($mb['idmembre']) ?>" class="btn btn-danger" >Delete</a></td>
			<td><a href="Saisiecompte.php?idmembre=<?php echo($mb['idmembre']) ?>" class="btn btn-primary" >CreerCompte</a>
